...
export type ConfirmationValidation = 'exact_13_or_15' | 'digits_only' | 'freeform';
...
export function normalizeConfirmationInput(value: string, settings: IdentifierSettings) {
  ...
  if (settings.confirmationValidation === 'exact_13_or_15') return digits.slice(0, 15);
...
export function validateConfirmation(value: string, settings: IdentifierSettings) {
  ...
  if (settings.confirmationValidation === 'exact_13_or_15' && normalized.length !== 13 && normalized.length !== 15) {
    return `${settings.confirmationLabel} must be 13 or 15 digits`;
  }
