...
async function publicBaseUrlForRow(supabaseServer: any, row: any) {
  const direct = normalizeBaseUrl(row?.public_hostname);
  if (direct) return direct;

  const processorId = String(row?.processor_id || '').trim();
  if (processorId) {
    try {
      ...
      console.warn('Processor public hostname lookup failed; falling back to SITE_URL.', error);
    }
  }
...
function processorContextFromRow(row: any): ProcessorContext | null {
  const id = String(row?.processor_id || '').trim();
  return id ? { id, slug: String(row?.processor_slug || '') } : null;
}

async function getNotificationBranding(row?: any) {
  const settings = await getPublicSiteSettings(null, processorContextFromRow(row));
  return {
...
type NotificationBalanceOpts = {
  paidProcessing?: boolean;
  processingPrice?: number;
  amountPaidProcessing?: number;
  paidSpecialty?: boolean;
  specialtyPrice?: number;
  amountPaidSpecialty?: number;
};

type NotificationTemplateOpts = NotificationBalanceOpts & {
  name?: string;
  tag?: string;
  link?: string;
  businessName: string;
...
function balanceDue(priceValue?: number, amountPaidValue?: number, paidFlag?: boolean) {
  const price = Math.max(0, Number(priceValue || 0));
  const amountPaid = Math.min(price, Math.max(0, Number(amountPaidValue || 0)));
  if (paidFlag || (price > 0 && amountPaid >= price)) return 0;
...
function notificationVars(opts: NotificationTemplateOpts) {
  ...
  const processingPrice = Math.max(0, Number(opts.processingPrice || 0));
  const specialtyPrice = Math.max(0, Number(opts.specialtyPrice || 0));
  const processingDue = balanceDue(processingPrice, opts.amountPaidProcessing, opts.paidProcessing);
  const specialtyDue = balanceDue(specialtyPrice, opts.amountPaidSpecialty, opts.paidSpecialty);
  return {
    ...
    statusUrl,
    statusLine: statusUrl ? `Status: ${statusUrl}` : '',
    pickupHours: String(opts.pickupHours || '').trim(),
    processingDueLine: opts.paidProcessing || (processingPrice > 0 && processingDue <= 0)
      ? 'Regular processing: PAID'
      : `Amount still owed (regular processing): $${processingDue.toFixed(2)}`,
    specialtyDueLine: opts.paidSpecialty || (specialtyPrice > 0 && specialtyDue <= 0)
      ? 'Specialty products: PAID'
      : `Amount still owed (specialty products): $${specialtyDue.toFixed(2)}`,
  };
}
...
function buildFinishedEmail(opts: NotificationTemplateOpts & { name: string; tag: string; paidProcessing: boolean; processingPrice: number; phoneDisplay: string }) {
  return buildNotificationEmail('meat_finished', opts);
}

function buildIntakeSms(opts: NotificationTemplateOpts & { tag: string; link?: string; statusUrl?: string }) {
...
function buildMeatFinishedSms(opts: NotificationTemplateOpts & { tag: string; paidProcessing: boolean; processingPrice: number; statusUrl: string }) {
  return buildNotificationSms('meat_finished', opts);
}

function buildCapeFinishedSms(opts: NotificationTemplateOpts & { tag: string; statusUrl: string }) {
  return buildNotificationSms('cape_finished', opts);
}

function buildSpecialtyFinishedSms(opts: NotificationTemplateOpts & { tag: string; paidSpecialty: boolean; specialtyPrice: number; statusUrl: string }) {
  return buildNotificationSms('specialty_finished', opts);
}

function buildWebbsDeliveredSms(opts: NotificationTemplateOpts & { tag: string; statusUrl: string }) {
  return buildNotificationSms('webbs_delivered', opts);
}

function buildCapeFinishedEmail(opts: NotificationTemplateOpts & { name: string; tag: string; phoneDisplay: string }) {
  return buildNotificationEmail('cape_finished', opts);
}

function buildSpecialtyFinishedEmail(opts: NotificationTemplateOpts & {
  name: string;
  tag: string;
  paidSpecialty: boolean;
  specialtyPrice: number;
...
function buildWebbsDeliveredEmail(opts: NotificationTemplateOpts & { name: string; tag: string; phoneDisplay: string }) {
...
export async function ensurePublicToken(supabaseServer: any, row: any) {
  ...
  const { data: updated, error } = await withProcessorFilter(
    supabaseServer
      ...
      .eq('id', row.id)
      .or('public_token.is.null,public_token.eq.')
      .select('public_token'),
    row?.processor_id ? String(row.processor_id) : null
  ).maybeSingle();
...
async function trySendDropoffEmail(supabaseServer: any, row: any) {
  ...
  const link = intakeFormLink(String(locked.tag || ''), String(locked.public_token || token), baseUrl);
...
async function trySendDropoffSms(supabaseServer: any, row: any) {
  ...
  const { data: locked } = await withProcessorFilter(
    supabaseServer
    .from('jobs')
    .select('id, processor_id, tag, phone, public_token')
    .eq('id', row.id)
...
async function loadSpecialtyItemsForRow(supabaseServer: any, row: any) {
  const jobId = String(row?.id || '').trim();
  if (!jobId) return [];
  const specialtyItemsMap = await loadJobSpecialtyItemsMap(supabaseServer, [jobId]);
  return specialtyItemsMap.get(jobId) || [];
}

async function rowWithSpecialtyItems(supabaseServer: any, row: any) {
  const items = await loadSpecialtyItemsForRow(supabaseServer, row);
  return items.length ? { ...row, specialtyItems: items } : row;
}

async function hasSpecialtyOrderForRow(supabaseServer: any, row: any) {
  if (!!row?.specialty_products) return true;
  if (specialtyTotalLbs(row as Record<string, any>) > 0) return true;
  const items = await loadSpecialtyItemsForRow(supabaseServer, row);
  return items.some((item) => Number(item.quantity ?? 0) > 0);
}

async function calcSpecialtyPriceForRow(supabaseServer: any, row: any, pricing?: Partial<SitePricing> | null) {
  return calcSpecialtyPriceFromLbs(await rowWithSpecialtyItems(supabaseServer, row), pricing);
}
...
async function trySendMeatFinishedEmail(supabaseServer: any, row: any) {
  ...
  const { data: locked } = await withProcessorFilter(
    supabaseServer
    .from('jobs')
    .select('id, processor_id, tag, email, customer_name, paid_processing, price_processing, amount_paid_processing, process_type, beef_fat, webbs_order')
    .eq('id', row.id)
...
async function trySendMeatFinishedSms(supabaseServer: any, row: any) {
  ...
  const { data: locked } = await withProcessorFilter(
    supabaseServer
    .from('jobs')
    .select('id, processor_id, tag, phone, paid_processing, price_processing, amount_paid_processing, process_type, beef_fat, webbs_order')
    .eq('id', row.id)
...
async function trySendSpecialtyFinishedEmail(supabaseServer: any, row: any) {
  if (!row || !hasRealTag(row) || row.requires_tag) return;
  if (!(await hasSpecialtyOrderForRow(supabaseServer, row)) || !specialtyReady(row.specialty_status)) return;
...
export async function resendCustomerNotification(params: {
  ...
  if (event === 'specialty_finished' && (!(await hasSpecialtyOrderForRow(supabaseServer, row)) || !specialtyReady(row.specialty_status))) {
  ...
  if (channel === 'email') {
    ...
    if (event === 'dropoff_tagged') {
      const token = await ensurePublicToken(supabaseServer, row);
      const link = intakeFormLink(String(row.tag || ''), token, baseUrl);
  ...
  if (event === 'dropoff_tagged') {
    ...
    const link = intakeFormLink(String(row.tag || ''), token, baseUrl);
    const branding = await getNotificationBranding(row);
    body = buildIntakeSms({ tag: String(row.tag || ''), link, statusUrl: link, businessName: branding.businessName, notificationTemplates: branding.notificationTemplates });
  }
  if (event === 'meat_finished') {
    const computed = calcProcessingPrice(row.process_type, !!row.beef_fat, !!row.webbs_order, await getCurrentPricing());
    const price = Number(row.price_processing ?? 0) || computed;
...
export async function logCall(params: {
  tag: string;
  scope?: 'meat' | 'cape' | 'specialty' | 'webbs';
  reason?: string;
  notes?: string;
  outcome?: string;
  ...
  try {
    if (scope === 'specialty') throw { code: '42883' };
    const payload = {
      p_tag: tag,
      p_scope: scope ?? null,
  ...
  const { error: updateError } = await withProcessorFilter(
    supabaseServer
    .from('jobs')
    .update(patch)
    .eq('id', job.id),
...
export async function markCalled(params: {
  tag: string;
  scope?: 'meat' | 'cape' | 'specialty' | 'webbs' | 'all' | 'auto';
  notes?: string;
  processorContext?: ProcessorContext | null;
}) {
  const supabaseServer = getSupabaseServer();
  ...
  const specialtyIsReady = specialtyReady(job.specialty_status);

  let scope = (rawScope || 'auto') as 'meat' | 'cape' | 'specialty' | 'webbs' | 'all' | 'auto';
  const updates: any = {};
  ...
  if (scope === 'all') {
    ...
    if (specialtyIsReady) updates.specialty_status = 'Called';
    ...
    updates.specialty_status = 'Called';
  } else if (scope === 'webbs') {
  ...
  if (Object.keys(updates).length > 0) {
    ...
    const { error: updErr } = await withProcessorFilter(
      supabaseServer
      .from('jobs')
      .update(updates)
      .eq('id', job.id),
...
export async function deletePendingJob(params: { jobId: string; processorContext?: ProcessorContext | null }) {
  const supabaseServer = getSupabaseServer();
  const processor = params.processorContext ?? await getDefaultProcessorContext();
...
export async function markIntakeSheetPrinted(params: { tag: string; processorContext?: ProcessorContext | null }) {
  const supabaseServer = getSupabaseServer();
  const processor = params.processorContext ?? await getDefaultProcessorContext();
  const tag = String(params.tag || '').trim();
  if (!tag) {
...
export async function markIntakeSheetUnprinted(params: { tag: string; processorContext?: ProcessorContext | null }) {
  const supabaseServer = getSupabaseServer();
  const processor = params.processorContext ?? await getDefaultProcessorContext();
  const tag = String(params.tag || '').trim();
  if (!tag) {
...
export async function listRemovedPendingJobs(opts: { processorContext?: ProcessorContext | null } = {}): Promise<{ ok: boolean; rows: JobSearchRow[] }> {
  const supabaseServer = getSupabaseServer();
  const processor = opts.processorContext ?? await getDefaultProcessorContext();
...
export async function restorePendingJob(params: { jobId: string; processorContext?: ProcessorContext | null }) {
  const supabaseServer = getSupabaseServer();
  const processor = params.processorContext ?? await getDefaultProcessorContext();
  const jobId = String(params.jobId || '').trim();
  if (!jobId) {
...
export async function listJobsNeedingPrint(opts: { processorContext?: ProcessorContext | null } = {}): Promise<{ ok: boolean; rows: JobSearchRow[] }> {
  const supabaseServer = getSupabaseServer();
  const processor = opts.processorContext ?? await getDefaultProcessorContext();
...
export async function setJobTag(params: {
  ...
  const { data: existing, error: existErr } = await withProcessorFilter(
    supabaseServer
    .from('jobs')
    .select('id, tag')
    .eq('tag', tag)
