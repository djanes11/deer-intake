...
import { getDashboardCounts, DashboardCounts, Trio } from '@/lib/api';

const lc = (v: any) => String(v ?? '').trim().toLowerCase();
const get = (r: any, ...keys: string[]) => { for (const k of keys) if (r?.[k] != null) return r[k]; };
const isReadyToCall = (s?: any) => {
  const v = lc(s);
  if (!v) return false;
  if (v.includes('called')) return false;
  return v.includes('ready') || v.includes('finish') || v.includes('finished') || v.includes('& ready') || v.includes('ready for pickup');
};
...
export default function Home() {
  ...
  // Client-side fallbacks to keep numbers populated even if GAS search names drift
  async function computeReadyFallback(current: DashboardCounts): Promise<DashboardCounts> {
  ...
  async function computeCalledFallback(current: DashboardCounts): Promise<DashboardCounts> {
    const allZero = (t: Trio) => (t.meat | t.cape | t.webbs) === 0;
    ...
    try {
      ...
      const j = await r.json();
      const rows: any[] = Array.isArray(j?.rows) ? j.rows : Array.isArray(j) ? j : [];
      const isCalled = (s?: any) => lc(s) === 'called' || lc(s).startsWith('called');
  ...
  async function refresh() {
    setBusy(true);
    const base = await getDashboardCounts();
    ...
    return (
      <span
        style={{
          ...
          padding: '6px 12px', borderRadius: 999, background: map.bg, border: `1px solid ${map.bd}`,
          fontWeight: 800, whiteSpace: 'nowrap', minWidth: 84, justifyContent: 'center',
        }}
        aria-label={`${label}: ${n}`}
      >
  ...
  const busyOverlay: React.CSSProperties = {
    ...
    background: 'rgba(0,0,0,.25)', borderRadius: 14, backdropFilter: 'blur(1px)',
    ...
    <div style={{ position: 'relative' }}>
      <div style={{ ...card, gridColumn: 'span 2' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 2 }}>
          ...
          <button onClick={refresh} className="btn" aria-label="Refresh dashboard" disabled={busy} style={{ padding: '6px 12px', opacity: busy ? 0.7 : 1 }}>
        ...
        <div style={rowFirst}>
          ...
          <Link href="/reports/calls" style={linkStyle}><div style={{ fontWeight: 800 }}>Call Report — Ready to Call</div></Link>
        ...
        <div style={row}>
          ...
          <Link href="/overnight/review" style={linkStyle}><div style={{ fontWeight: 800 }}>Overnight — Missing Tag</div></Link>
          {chip('Open', counts.needsTag, 'amber')}
        </div>

        <div style={row}>
          <div style={{ width: 12, height: 12, borderRadius: 999, background: 'rgba(115,75,170,.95)' }} />
          <Link href="/reports/called" style={linkStyle}><div style={{ fontWeight: 800 }}>Called — Pickup Queue</div></Link>
          <div style={{ display: 'flex', gap: 10 }}>
  ...
  return (
    <main className="watermark" style={shell}>
      <div style={header}>
        <div style={{ color: '#89c096', fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', fontSize: 12, marginBottom: 6 }}>Welcome</div>
      ...
      <div style={{ ...trio, marginBottom: 16 }}>
        <Link href="/intake" style={{ textDecoration: 'none', color: 'inherit' }}>
          <div style={card}><div style={mini}>Intake</div><div style={{ fontWeight: 900, fontSize: 18, marginTop: 6 }}>New Intake form</div><div style={{ opacity: 0.8, marginTop: 4 }}>Start a new Intake Form</div></div>
        </Link>
        <Link href="/scan" style={{ textDecoration: 'none', color: 'inherit' }}>
          <div style={card}><div style={mini}>Scan</div><div style={{ fontWeight: 900, fontSize: 18, marginTop: 6 }}>Scan Tags</div><div style={{ opacity: 0.8, marginTop: 4 }}>Update status by scanning a barcode</div></div>
        </Link>
        <Link href="/search" style={{ textDecoration: 'none', color: 'inherit' }}>
          <div style={card}><div style={mini}>Search</div><div style={{ fontWeight: 900, fontSize: 18, marginTop: 6 }}>Search Jobs</div><div style={{ opacity: 0.8, marginTop: 4 }}>Find by name, tag, or phone #</div></div>
        </Link>
      </div>
      ...
      <div style={trio}>
        ...
        <div style={card}>
          ...
          <Link href="/tips" style={{ textDecoration: 'none', color: 'inherit' }}><div style={{ fontWeight: 900, fontSize: 18, marginTop: 6 }}>Tip Sheet</div></Link>
          <div style={{ opacity: 0.8, marginTop: 4 }}>Short reminders for staff</div>
          <div style={{ height: 10 }} />
          <Link href="/faq" style={{ textDecoration: 'none', color: 'inherit' }}><div style={{ fontWeight: 900, fontSize: 18, marginTop: 6 }}>FAQ</div></Link>
