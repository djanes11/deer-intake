...
function gFuzzy(row: any, tokenSets: string[][]): { key: string; value: string } | null {
  ...
  for (const tokens of tokenSets) {
    const hit = lc.find(([lk, v]) => tokens.every(t => lk.includes(t)) && String(v ?? '').trim() !== '');
    if (hit) {
...
function findConfirmationField(row: any): { key: string; value: string } | null {
  // common exacts first
  const exacts = ['Confirmation #','Confirmation','confirmation','Confirm #','Confirm#','Confirmation#'];
  for (const k of exacts) {
    const v = row?.[k];
    if (hasCell(v)) return { key: k, value: norm(v) };
  }
  // fuzzy fallback: any key containing "confirm"
...
function toPublic(row: any) {
  ...
  // raw track cells (NO remapping; show exactly what’s in the sheet)
...
export async function POST(req: NextRequest) {
  ...
  // 1) Fast path: Tag + Last Name → 1 call (job)
  ...
  // 2) Confirmation path — at most TWO calls (search → job), with one broad fallback
  if (confirmation) {
    let rows: any[] = [];

    // Try direct search with the given string
    {
      const data = await gas(origin, { action: 'search', q: confirmation });
      if (Array.isArray(data?.rows)) rows = data.rows;
    }

    // If not matched, one broad fallback '@all'
    ...
    if (!match) {
      const data2 = await gas(origin, { action: 'search', q: '@all' });
    ...
    if (tag) {
      const j2 = await gas(origin, { action: 'job', tag: tag });
      const job = j2?.job;
      if (job) {
        return new Response(JSON.stringify(toPublic(job)), { headers:{ 'Content-Type': 'application/json' } });
      }
    ...
    // Fallback to search row if /job fails
